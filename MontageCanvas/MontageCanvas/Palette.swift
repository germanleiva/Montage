...
{
    static let initialWidth = CGFloat(400) // UIScreen.main.bounds.width
    @objc open weak var delegate: PaletteDelegate?
    fileprivate var brush: Brush = Brush()
    
    fileprivate let buttonDiameter = initialWidth / 10.0
    fileprivate let buttonPadding = initialWidth / 25.0
    ...
    fileprivate func setupDefaultValues() {
        ...
        onClickColorPicker(self.colorButtonList[4]) //Added to select red as the starting color
    }
